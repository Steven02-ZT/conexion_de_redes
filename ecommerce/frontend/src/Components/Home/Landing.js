...
import { BsCircle,BsFillCircleFill,BsArrowRightShort } from 'react-icons/bs'
...
function Lanfing() {
  ...
  const news = useSelector((state) => state.newest)
  const {error,newest} = news
  ...
  let only4Offerts = []
  try{
    only4Offerts = Object.values(newest).slice(0, 4);
  }catch{}

  return (
    <div className="h-screen w-full bg-landing">
      <div className="h-2/6 w-full flex justify-center items-center" style={{"backgroundColor":"rgba(255, 255, 255, 0.1)"}}>
        <div className="text-center">
          <h1 className="text-4xl font-bold text-slate-700 p-2">Bienvenidos al futuro</h1>
          <h3 className="text-2xl font-bold text-slate-600 p-2">del Data Center</h3>
        </div>
      </div>
      <div className="h-4/6 w-full grid grid-cols-4 gap-2 justify-center relative bg-gradient-landing pr-2 pl-2">
          {/*cards*/}
          {newest ? only4Offerts.map(element => (
        ...
        <div className="w-full flex justify-center buttonLink bg-white">
        <Link to={'/productos?type=nuevos'}
        className='bg-slate-400 text-white rounded-md col-span-4 m-auto p-3 text-center relative lg:w-1/6 md:w-2/6'>
          Explorar todos <BsArrowRightShort className="inline text-2xl"/>
        </Link>
        </div>
    </div>
  );
