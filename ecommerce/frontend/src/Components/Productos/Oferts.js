...
function ProductosLista() {
  ...
  const { error, loading, offerts } = data;
  ...
  return (
    <div>
        <h1 className="text-2xl text-blue-900 font-bold pl-5">Mira nuestras ofertas!! </h1>
    <div className="w-full grid grid-cols-4 gap-2 justify-center relative bg-gradient-landing pt-10 pb-10 gridProductsList">
      {loading ? (
        ...
        <div key={element.id} className="h-4/5 rounded-xl ml-auto mr-auto mb-5 card" >
        <div className="h-2/3 rounded-xl card-image" style={{ backgroundImage:`URL(${element.images[0].path})`, }} ></div>
